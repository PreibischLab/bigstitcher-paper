...
\vspace{-2.0mm}
\caption{\hspace{-0.5mm} \red{\emph{Automatic Quantification of Image Quality.} \textbf{(a-d)} Four different z-planes from a volume that overlays the results of the relative Fourier Ring Correlation (rFRC, see \textbf{Online Methods}) computed in 128$\times$128 blocks using a spacing of 10 pixels (magenta) and cleared image data (same as \textbf{Supplementary Fig. \ref{fig:sup-fig-rf}}). The rFRC robustly detects areas with high image quality. Note that (a) is deepest inside the tissue and (d) is at the surface of the sample. See \textbf{Supplementary Video 8} for an animation of the entire stack. The rFRC was successfully applied to all cleared datasets in this publication (\textbf{Supplementary Table 1}), results are also shown in \textbf{Supplementary Fig. \ref{fig:sup-fig-rf}, \ref{fig:sup-rfrc-brain}} and \textbf{Supplementary Video 8,9}.
}}
\label{fig:sup-rFRC}
\end{figure*}
